...
export default function POSInterface() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // 3. Success
      showToast('Thanh toÃ¡n thÃ nh cÃ´ng! ÄÃ£ ghi nháº­n Ä‘Æ¡n hÃ ng.');
      setCart([]);
    } catch (err) {
      console.error(err);
      showToast('Lá»—i thanh toÃ¡n, vui lÃ²ng thá»­ láº¡i!', 'error');
    } finally {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex gap-8 animate-in fade-in slide-in-from-right-4 duration-500 relative">
      {/* Toast Notification */}
      {toast && (
      ...
      <div className="flex-1 flex flex-col bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
         <div className="p-6 border-b border-slate-50 flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Thá»±c Ä‘Æ¡n</h2>
              <button 
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredProducts.map(product => (
                  ...
                  >
                    ...
                    <p className="text-sm font-black text-slate-900 mt-2">{Intl.NumberFormat('vi-VN').format(product.price)}Ä‘</p>
                  </button>
                ))}
              </div>
            )}
         ...
         <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex items-center justify-between">
               <div className="flex items-center gap-2">
                 ...
                 <h2 className="text-lg font-black text-slate-900 italic">Giá» hÃ ng</h2>
               </div>
               <span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg">{cart.length} mÃ³n</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
               {cart.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-slate-300">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest">Giá» hÃ ng Ä‘ang trá»‘ng</p>
                 </div>
               ) : (
                 cart.map(item => (
                   <div key={item.id} className="bg-slate-50 p-4 rounded-2xl flex items-center gap-3 group">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-xs font-bold text-indigo-600">{Intl.NumberFormat('vi-VN').format(item.price)}Ä‘</p>
                      </div>
            ...
            <div className="p-6 bg-slate-900 text-white rounded-b-[2.5rem]">
               <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Táº¡m tÃ­nh</p>
                  <p className="text-sm font-black">{Intl.NumberFormat('vi-VN').format(calculateTotal())}Ä‘</p>
               </div>
               <div className="flex items-center justify-between mb-6">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Tá»•ng cá»™ng</p>
                  <p className="text-2xl font-black text-indigo-400 italic leading-none">{Intl.NumberFormat('vi-VN').format(calculateTotal())}Ä‘</p>
               </div>
